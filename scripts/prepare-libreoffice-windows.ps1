...
$Soffice = Get-ChildItem -Path $CandidateRoots -Recurse -Filter "soffice.exe" |
  Where-Object { $_.FullName -match "[\\/]program[\\/]soffice\.exe$" } |
  Select-Object -First 1

if (-not $Soffice) {
  throw "Could not locate LibreOffice program\soffice.exe after installation."
}
...
if (-not (Test-Path $BundledSoffice)) {
  throw "Bundled soffice.exe was not copied to $BundledSoffice."
}
...
& $BundledSoffice --headless "-env:UserInstallation=$SmokeProfileUrl" --convert-to pdf --outdir $SmokeDir $SmokeInput
if ($LASTEXITCODE -ne 0) {
